    ...
    <section>
        ...
        <ul>
            <?php if(is_array($projects)): $i = 0; $__LIST__ = $projects;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$project): $mod = ($i % 2 );++$i;?><li>
                    ...
                    <span ><a href="/index.php/Home/Projectfind/send_pid/pid/<?php echo ($project["pro_id"]); ?>"><?php echo ($project["pro_title"]); ?></a></span>
...
<div class="consultDiv">
    <form class="reg" action="<?php echo U('Projectfind/issue');?>" method="post">
        <div>标题：<input class="title" name="title" type="text" placeholder="标题" type="text" position="relative" ></div>
        <div>内容：</div>
        <div><textarea class="content" name="content" type="text" value="" rows="3" cols="26" placeholder="内容" style="left: 46px; padding: 10px; position: relative; margin: 0px; width: 652px; height: 102px;"></textarea></textarea></div>
