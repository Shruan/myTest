...
<div class="clearfix2" style="height: 280px">
    <section style="width: 88%">
        <h4>详细内容</h4>
        <ul>
            <?php if(is_array($projects)): $i = 0; $__LIST__ = $projects;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$project): $mod = ($i % 2 );++$i;?><li style="width: 99%;height: 35px">
                    <span ><h2><?php echo ($project["pro_title"]); ?></h2></span>
                    <div style="border: 1px solid rgba(125, 114, 114, 0.5);height: 206px;">
                        <span class="d_user" rel="<?php echo ($project["pro_belong_user"]); ?>"><h3><?php echo ($project["pro_belong_user"]); ?></h3></span>
                        <span class="d_cont"><?php echo ($project["pro_content"]); ?></span>
                        <span class="d_date"><?php echo ($project["pro_date"]); ?></span>
                    </div>
                </li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
    </section>
    <label class="label1" rel="<?php echo ($project["pro_acc_user"]); ?>"></label>
</div>
<div style=" position: relative;  padding-left: 44%;">
    <button class="btn_style pbtn1" style="display: none" rel="<?php echo ($project["pro_id"]); ?>">接取</button>
    <button class="btn_style pbtn2" style="display: none" rel="<?php echo ($project["pro_id"]); ?>">终止</button>
    <button class="btn_style pbtn3" style="display: none" rel="<?php echo ($project["pro_id"]); ?>">完成</button>
</div>
...
<script>
    //获取登录ID和接取委托ID
    var loginID=$('.loginId').attr('rel');
    var isacc=$('.label1').attr('rel');
    var isuseID=$('.d_user').attr('rel');

//    alert("登录ID"+loginID+"   接取ID"+isacc+"   发布ID"+isuseID );

    $.ajax({
        url: '/index.php/Home/Project/is_acc',
        type: 'POST',
        data:{
            pid: $('.pbtn1').attr('rel'),
        },
        ...
        success: function(jsonResult){
            ...
            if(jsonResult.pro_complete==1){
                $('.label1').html('委托已完成');
                $('.pbtn2').css('display','none');
                $('.pbtn1').css('display','none');
                $('.pbtn3').css('display','none');
                return;
            }
            if(jsonResult.pro_acc_user==''||jsonResult.pro_acc_user==null){
                $('.label1').html("委托还未被接取");
                $('.pbtn1').css('display','block');
                return;
            }
            if(jsonResult.pro_acc_user!=undefined && jsonResult.pro_complete==0){
                $('.label1').html('委托已被'+jsonResult.pro_acc_user+"用户接取,未完成");
                if(loginID==''){
                    $('.pbtn1').css('display','none');
                    $('.pbtn2').css('display','none');
                    $('.pbtn3').css('display','none');
                ...
                if(loginID==isacc){
                    $('.pbtn2').css('display','block');
                }
                if(loginID==isuseID){
                    $('.pbtn2').css('display','block');
                    $('.pbtn3').css('display','block');
                }
                return;
            }
        ...
        $('.pbtn1').click(function(){
            ...
            $.ajax({
                url: '/index.php/Home/Project/acc_pid',
                type: 'POST',
                data:{
                    pid: $(this).attr('rel'),
                },
        ...
        $('.pbtn2').click(function(){
            $.ajax({
                url: '/index.php/Home/Project/quit_pid',
                type: 'POST',
                data:{
                    pid: $(this).attr('rel'),
                },
    ...
    $('.pbtn3').click(function(){
        $.ajax({
            url: '/index.php/Home/Project/over_pid',
            type: 'POST',
            data:{
                pid: $(this).attr('rel'),
            },
