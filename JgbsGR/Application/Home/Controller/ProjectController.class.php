...
//        print_r($session_users);
        ...
        $projects = M('project')->where('pro_type=1 AND pro_examine=1')->select();
    ...
    public function issue(){
        ...
        alert('发布成功，等待审核', 'success', U('project/all_project'));
    }
...
//    }
    ...
    //点击委托标签发送的ID跳转页面，<a href="__URL__/send_pid/pid/{$project.pro_id}">{$project.pro_title}</a>
    public function send_pid(){
        ...
        $pid=I('get.pid');
        $projects = M('project')->where("pro_id='$pid'AND pro_type=1 AND pro_examine=1")->select();
        $this->assign('projects',$projects);
...
//        print_r($projects);
    ...
    public function is_acc(){
        $pid=$_POST['pid'];
        $projects = M('project')->where("pro_id='$pid'AND pro_type=1 AND pro_examine=1")->find();
    ...
    public function over_pid(){
        ...
        $count=$project->where("pro_acc_user='$acc_users' and pro_complete=1 and pro_type=1")->count();
        $user=M('user');
