...
//        print_r($session_users);
        ...
        $projects = M('project')->where('pro_type=4 AND pro_examine=1')->select();
    ...
    public function issue(){
        ...
        alert('发布成功，等待审核', 'success', U('projectad/all_projectad'));
    }
...
//    }
    ...
    public function send_pid(){
        ...
        $pid=I('get.pid');
        $projects = M('project')->where("pro_id='$pid'AND pro_type=4 AND pro_examine=1")->select();
        $this->assign('projects',$projects);
...
//        print_r($projects);
    ...
    public function is_acc(){
        $pid=$_POST['pid'];
        $projects = M('project')->where("pro_id='$pid'AND pro_type=4 AND pro_examine=1")->find();
    ...
    public function over_pid(){
        ...
        $count=$project->where("pro_acc_user='$acc_users' and pro_complete=1 and pro_type=4")->count();
        $user=M('user');
